...
function normalizeSong(raw = {}) {
  ...
  return {
    id, schemaVersion: SCHEMA_VERSION, title: cleanText(raw.title, 160), artist: cleanText(raw.artist, 160),
...
function renderFeatured() {
  const song = songs[0]; const section = $('#featured');
  if (!song) { section.innerHTML = `<div class="featured-record"><div class="featured-sleeve"><div class="art-surface"><div class="generated-art" data-initials="01" style="--art-a:#4d4f45;--art-b:#20211e"></div></div></div><div class="featured-copy"><p class="catalog">Now shelving · 001</p><h2>Your first<br>record</h2><p class="artist">The shelf is waiting</p><blockquote>“从一首你舍不得忘记的歌开始。”</blockquote><div class="featured-actions"><button class="primary-button" data-add type="button">Add a record</button></div></div></div>`; return; }
  section.innerHTML = `<article class="featured-record"><div class="featured-sleeve">${artworkSurface(song)}${fingerprintSvg(song)}</div><div class="featured-copy"><p class="catalog">Now on the turntable · 001 / ${String(songs.length).padStart(3,'0')}</p><h2>${escapeHtml(song.title)}</h2><p class="artist">${escapeHtml(song.artist)}${song.album ? ` · ${escapeHtml(song.album)}` : ''}</p><blockquote>“${escapeHtml(song.reflection)}”</blockquote><div class="featured-actions"><button class="primary-button" data-open="${escapeHtml(song.id)}" type="button">Open record</button><button class="text-button" data-favorite="${escapeHtml(song.id)}" type="button">${song.favorite ? '♥ Cherished' : '♡ Cherish'}</button></div></div></article>`;
}
...
function render() {
  ...
  grid.innerHTML = visible.map((song,index) => `<article class="song-card" tabindex="0" data-id="${escapeHtml(song.id)}" aria-label="打开 ${escapeHtml(song.title)} 的唱片"><div class="sleeve-stage">${fingerprintSvg(song)}${artworkSurface(song)}</div><div class="card-topline"><div><p class="eyebrow">SC-${String(index+1).padStart(3,'0')}</p><h3>${escapeHtml(song.title)}</h3><p class="artist">${escapeHtml(song.artist)}${song.album?` · ${escapeHtml(song.album)}`:''}</p></div><button class="heart ${song.favorite?'active':''}" data-favorite="${escapeHtml(song.id)}" type="button" aria-label="${song.favorite?'取消珍藏':'标为珍藏'}" aria-pressed="${song.favorite}">${song.favorite?'♥':'♡'}</button></div><p class="reflection-preview">“${escapeHtml(song.reflection)}”</p><div class="tags">${song.tags.slice(0,3).map(tag=>`<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div></article>`).join('');
...
function openExhibit(song, discovered=false) {
  ...
  $('#exhibitContent').innerHTML=`<button class="close-button" data-close type="button" aria-label="关闭">×</button><div class="exhibit-layout"><div class="exhibit-visual">${artworkSurface(song)}${fingerprintSvg(song)}</div><div class="exhibit-body"><p class="eyebrow">${discovered?'You found this again':'Personal pressing'} · ${new Date(song.createdAt).toLocaleDateString('zh-CN')}</p><h2>${escapeHtml(song.title)}</h2><p class="artist">${escapeHtml(song.artist)}${song.album?` · ${escapeHtml(song.album)}`:''}</p>${discovered?`<p class="status-message">你在 ${days} 天前把它留在了这里。</p>`:''}<p class="big-reflection">“${escapeHtml(song.reflection)}”</p>${song.note?`<p class="note">${escapeHtml(song.note)}</p>`:''}${associations.length?`<dl class="meta-list">${associations.map(([k,v])=>`<dt>${assocLabels[k]}</dt><dd>${escapeHtml(v)}</dd>`).join('')}</dl>`:''}<div class="tags">${song.tags.map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join('')}</div><div class="exhibit-actions"><button class="primary-button" data-favorite="${escapeHtml(song.id)}" type="button">${song.favorite?'取消珍藏':'♡ 标为珍藏'}</button><button class="text-button" data-edit="${escapeHtml(song.id)}" type="button">编辑</button><button class="text-button danger" data-delete="${escapeHtml(song.id)}" type="button">删除</button></div></div></div>`;
